...
module.exports = {
  async index(request, response) {
    ...
    if (request.query.search) {
      ...
      let filteredModule = search
        ? unity.filter((unity) => unity.name.includes(search))
        : unity;
      // console.log(request.query.search ? 'foi' : 'n達o foi')

      filteredModule = filteredModule.map((m) => ({
        ...m,
        image_url: '192.168.3.7:3333/uploads/' + m.image_name,
      }));
  ...
  async indexOneById(request, response) {
    ...
    if (request.query.search) {
      ...
      const filteredModule = search
        ? unity.filter((unity) =>
            unity.name.toLowerCase().includes(search.toLowerCase()),
          )
        : unity;
      // console.log(request.query.search ? 'foi' : 'n達o foi')
      return response.json(filteredModule);
  ...
  async create(request, response) {
    ...
    const image_url = `192.168.3.7:3333/uploads/${request.file.filename}`;
  ...
  async update(request, response) {
    ...
    await connection('unity')
      ...
      .update(
        ...
        [image_url, name, content, module_id],
      );
